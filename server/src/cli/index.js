 ...
 */
function displayPlayers(engine, players, title = 'Select a player:') {
  console.log(colorize(title, colors.bright));
  players.forEach((player, idx) => {
    const fatigueState = engine.fatigue.getState(player.id);
    let fatigueIndicator = '';

    if (fatigueState.penaltyPending) {
      fatigueIndicator = colorize(' [TIRED!]', colors.red);
    } else if (fatigueState.count > 0) {
      fatigueIndicator = ` [${fatigueState.count}/3]`;
    }
 ...
 */
async function selectPlayer(rl, engine, players, promptMsg) {
  displayPlayers(engine, players, promptMsg);
...
function displayLastEvent(engine) {
  ...
  if (lastEvent.contestResult) {
    ...
    let outcomeText = 'LOSS';
    if (cr.diff >= 5) outcomeText = 'DECISIVE';
    else if (cr.diff > 0) outcomeText = 'WIN';
    else if (cr.diff === 0) outcomeText = 'TIE';
    console.log(`Outcome: ${outcomeText} (diff: ${cr.diff})`);
  }
  ...
  try {
    ...
    while (!engine.isFinished()) {
      ...
      if (zone < Zones.ATTACK) {
        ...
        // Resolve shot
        const result = engine.resolveShot({ // Used below for goal check
          shooter,
          opponentGK: gk,
          context: { useSuperMove }
        });
