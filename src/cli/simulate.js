 ...
 */
function runSimulations(count = 10) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const result = simulateMatch(seed, false);
...
if (mode === 'single') {
  ...
  // Pass flags through to each match
  console.log(`Running ${count} matches with options: roundsPerHalf=${flags.roundsPerHalf || process.env.ROUNDS_PER_HALF || 45}, forceFullMatch=${typeof flags.forceFullMatch !== 'undefined' ? flags.forceFullMatch : (process.env.FORCE_FULL_MATCH || true)}`);
  console.log('');

  for (let i = 0; i < count; i++) {
    const seed = Date.now() + i * 1000;
    simulateMatch(seed, false, flags);
  }
} else {
  console.log('Field Battle Soccer Simulator');
  console.log('');
